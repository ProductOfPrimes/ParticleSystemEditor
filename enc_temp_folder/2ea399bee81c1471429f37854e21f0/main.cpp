...
void OpenEmitterSystemTextFile() {
	...
	if (result == NFD_OKAY)
	{
		...
		if (textFile.is_open()) {
			...
			addEmitter();
			ParticleEmitter* emitter = activeSystem->m_emitters[0];
			ParticleEmitter::Config newConfig = ParticleEmitter::Config();
			Transform transform = Transform();
			textFile.read(reinterpret_cast<char*>(&newConfig), sizeof(ParticleEmitter::Config));
			textFile.read(reinterpret_cast<char*>(&transform), sizeof(Transform));
			emitter->myState.transform = transform;
			emitter->myConfig = newConfig;
			emitter->setNumParticles(newConfig.numberOfParticles);
